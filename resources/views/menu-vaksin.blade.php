...
<x-app-layout>
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          ...
          @endif
          <button id="closeDetailModalBtn" class="bg-gray-300 text-gray-800 px-4 py-2 rounded-md">Tutup</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function renderNotifTable() {
      ...
      let filtered = isDokter ? data : data.filter(item => item.patientEmail === currentEmail);

      if (filtered.length === 0) {
        notifBody.innerHTML = `<tr><td colspan="5" class="px-4 py-2 text-gray-500">Tidak ada data vaksinasi.</td></tr>`;
        return;
      }

      filtered.forEach((d, i) => {
        const encodedData = JSON.stringify(d).replace(/"/g, '&quot;');
        notifBody.insertAdjacentHTML('beforeend', `
          <tr>
    ...
    function saveNotification() {
      ...
      const newData = {
        vaccineName, vaccineDate, doctorName, address,
        patientEmail: "{{ Auth::user()->email }}", status: "Belum",
        timestamp: Date.now()
      };
      const data = JSON.parse(localStorage.getItem('vaccineNotifications')) || [];
      data.push(newData);
      localStorage.setItem('vaccineNotifications', JSON.stringify(data));
      closeAddModal(); renderNotifTable();
    }

    function showDetail(d) {
      document.getElementById('detail-nama-vaksin').textContent = d.vaccineName;
      document.getElementById('detail-tanggal').textContent = d.vaccineDate;
      document.getElementById('detail-nama-dokter').textContent = d.doctorName;
      document.getElementById('detail-nama-pasien').textContent = d.patientEmail;
      document.getElementById('detail-alamat').textContent = d.address;
      document.getElementById('detail-status').textContent = d.status;
      document.getElementById('detailModal').classList.remove('hidden');

      if (userRole === 'dokter') {
        document.getElementById('markAsDoneBtn').onclick = () => moveToHistory(d, 'Sudah');
        document.getElementById('markAsCanceledBtn').onclick = () => moveToHistory(d, 'Dibatalkan');
      }
    }

    function moveToHistory(data, status) {
      const history = JSON.parse(localStorage.getItem('vaccineHistory')) || [];
      data.status = status; history.push(data);
