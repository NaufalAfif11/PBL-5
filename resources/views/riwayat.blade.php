                    ...
                    <tbody class="bg-white divide-y divide-pink-100">
                        @forelse ($riwayat as $index => $item)
                            <tr class="hover:bg-pink-50">
                                ...
                                <td class="px-4 py-2 font-semibold {{ $item->status == 'Sudah' ? 'text-green-600' : 'text-red-600' }}">{{ $item->status }}</td>
                                <td class="px-4 py-2">
                                    <button onclick="showDetail({{ $item->id }})" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">Detail</button>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-4 py-2 text-gray-500">Tidak ada riwayat vaksinasi.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
